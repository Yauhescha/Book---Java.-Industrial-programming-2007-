...
//	(Характеристикой столбца прямо-угольной матрицы называется сумма модулей его элементов).
	public void task19(float arr[][]) {
		boolean next=true;
		while(next) {
			next=false;
			for (int i = 0; i < arr[0].length-1; i++) {
				if(getSumColl(arr,i)<getSumColl(arr, i+1)) {
					next=true;
					arr=changeCollsMatrix(arr, i, i+1);
				}
			}
		}
		printArray(arr);
	...
	private float getSumColl(float[][] arr, int col) {
		float sum = 0;
		for (int i = 0; i < arr.length; i++) {
			sum += arr[i][col];
		}
...
//	20. Путем перестановки элементов квадратной вещественной матрицы до-биться того, чтобы ее максимальный элемент находился в левом верх-нем углу, 
//	следующий по величине – в позиции (2,2), следующий по ве-личине – в позиции (3,3) и т. д., заполнив таким образом всю главную диагональ.
	public void task20(float arr[][]) {
		
		float diagonal[]=new float[arr[0].length];
		
		for (int diagonalIndex = 0; diagonalIndex < diagonal.length; diagonalIndex++) {
			float max =getMax(arr);
		label:for (int i = 0; i < arr.length; i++) {
				for (int j = 0; j < arr[i].length; j++) {
					if(arr[i][j]==max) {
						diagonal[diagonalIndex]=arr[i][j];
						arr[i][j]=-999999999f;
						break label;
					}
				}
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[i].length; j++) {
				if(arr[i][j]==-999999999) {
					arr[i][j]=arr[count][count];
					arr[count][count]=diagonal[count];
					count++;
				}
			}
		}
